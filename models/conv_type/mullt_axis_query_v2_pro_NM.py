...
class MaxQConv2dBNV2ProNM(BaseNMConv):
    def __init__(self, conv_type, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1,
        ...
        super(MaxQConv2dBNV2ProNM, self).__init__()
    ...
    def get_mask(self, weight):
        ...
        preserve_index = int(index - index * self.get_preserve_rate(cur_epoch, decay_start, decay_end))
        ...
        mask_local = mask_local.scatter_(dim=1, index=index_local[:, :preserve_index], value=0.)
...
class SoftMaxQConv2DBNV2ProNM(MaxQConv2dBNV2ProNM):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
                 padding_mode='zeros', **kwargs):
        super(SoftMaxQConv2DBNV2ProNM, self).__init__(SoftConv2D, in_channels, out_channels, kernel_size, stride=stride,
                                                      padding=padding, dilation=dilation, groups=groups, bias=bias,
                                                      padding_mode=padding_mode, **kwargs)


class HardMaxQConv2DBNV2ProNM(MaxQConv2dBNV2ProNM):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
                 padding_mode='zeros', **kwargs):
        super(HardMaxQConv2DBNV2ProNM, self).__init__(HardConv2D, in_channels, out_channels, kernel_size, stride=stride,
                                                      padding=padding, dilation=dilation, groups=groups, bias=bias,
                                                      padding_mode=padding_mode, **kwargs)

...
class MaxQConv2dBNV2Pro(BaseNMConv):
    def __init__(self, conv_type, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1,
        ...
        super(MaxQConv2dBNV2Pro, self).__init__()
    ...
    def get_preserve_rate(self, cur_epoch, decay_start, decay_end) -> float:
        ...
        if self.schedule == 'linear':
            ...
            preserve_rate = max(min((1 - (cur_epoch - decay_start) / (decay_end - decay_start))**3, 1), 0)
    ...
    def get_mask(self, weight):
        ...
        sorted_weight_nm, index_weight_nm = torch.sort(weight_nm.sum(dim=-1), dim=0, descending=False)
...
class SoftMaxQConv2DBNV2Pro(MaxQConv2dBNV2Pro):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
                 padding_mode='zeros', **kwargs):
        super(SoftMaxQConv2DBNV2Pro, self).__init__(SoftConv2D, in_channels, out_channels, kernel_size, stride=stride,
                                                    padding=padding, dilation=dilation, groups=groups, bias=bias,
                                                    padding_mode=padding_mode, **kwargs)


class HardMaxQConv2DBNV2Pro(MaxQConv2dBNV2Pro):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
                 padding_mode='zeros', **kwargs):
        super(HardMaxQConv2DBNV2Pro, self).__init__(HardConv2D, in_channels, out_channels, kernel_size, stride=stride,
                                                    padding=padding, dilation=dilation, groups=groups, bias=bias,
                                                    padding_mode=padding_mode, **kwargs)
